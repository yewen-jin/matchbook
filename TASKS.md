...
- [x] **Answer questions inline** - the questions alert previously had nowhere to actually respond; found during a live test where "asks, never guesses" had no way to answer back. Added a `?/answer` action (`src/routes/app/+page.server.ts`) — one text input per open question, submitting appends the answers as a "[Clarification from Caslean]" block to the conversation and re-runs extraction. Verified live: answering the vague-rate case's two questions (fee, year) correctly filled `unit_amount`/`event_date` with the right provenance, and correctly still asked about the one field that was never answered (payment terms) — the loop is iterative, not one-shot.
